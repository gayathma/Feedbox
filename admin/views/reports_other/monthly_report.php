        ...
        <div>
            <header class="panel-heading">
                <a href="<?php echo site_url(); ?>/login/load_login"><i
                        class="fa fa-arrow-left"></i>Monthly Feedback Report</a>
...
<script type="text/javascript">
    $(document).ready(function ($) {
        $("#month_report").validate({
            ...
            submitHandler: function(form) {
                $.post(site_url + '/reports/generate_monthly_report', $("#month_report").serialize(), function (msg) {
                    $('#report_content').html('');
    ...
    function export_report(){
        if($("#month_report").valid()){
            var win = window.open(site_url + '/reports/generate_monthly_report_export?month=' + $("#month").val()+'&total_admissions='+ $("#total_admissions").val()+'&questionnaire=' + $("#questionnaire").val(), '_blank');
            win.focus();
        }
    }
